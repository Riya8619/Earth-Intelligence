...
def _ai_provider() -> Optional[Dict[str, str]]:
    """Pick whichever AI provider is actually configured.

    The project ships a GROQ_API_KEY (and the `groq` package is listed in
    requirements.txt) but this module previously only ever called OpenAI,
    whose key is blank by default. That meant every briefing silently fell
    back to the static template. Prefer OpenAI if it's explicitly
    configured, otherwise use Groq's OpenAI-compatible endpoint.
    """
...
def generate_briefing(environment: Dict[str, Any], earth_health_score: int, query: Optional[str] = None) -> Dict[str, Any]:
    ...
    if requests is None:
        logger.error("requests package is unavailable; cannot call the AI provider")
        raise RuntimeError("requests package is unavailable")

    provider = _ai_provider()
    if provider is None:
        logger.warning("No AI provider API key is configured; skipping AI call")
    ...
    try:
        ...
        logger.exception("AI briefing generation failed: %s", exc)
